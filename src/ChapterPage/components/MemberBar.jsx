...
function Meeting({
    ...
    return (
      <div className="sticky top-0 flex justify-around bg-gradient-to-r from-yellow-300 to-yellow-400 shadow-md rounded-t-2xl py-4 px-2 select-none w-fit lg:w-full xl:w-full">
        {headers.map((element, index) => (
          <p
            key={index}
            className="min-w-[120px] font-semibold text-gray-900 dark:text-gray-100 text-center mx-2 tracking-wide text-nowrap"
          >
            {element}
          </p>
        ))}
  ...
  return (
    <div className="flex justify-around items-center py-3 px-2 w-max lg:w-full xl:w-full cursor-auto rounded-b-lg transition-colors hover:bg-yellow-50 dark:hover:bg-yellow-900 even:bg-gray-50 dark:even:bg-gray-700 odd:bg-gray-100 dark:odd:bg-gray-800 shadow-sm select-text">
